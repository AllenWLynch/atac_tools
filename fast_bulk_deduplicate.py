    ...
    def flush_memory(self):
        ...
        for location_id, barcodes in self.fragment_memory.items():
            ...
            if len(barcodes) > 1 and self.mark:
                ...
                print('\t'.join([*location_id, barcodes[0], '-1']))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-n','--numdups',action = 'store_true', default=False, help = 'Record the number of duplicates oberserved for each fragment')
    ...
    for linenum, fragment in enumerate(args.input.readlines()):
        ...
        try:
            ...
            print('ERROR: line #{} has incorrect number of fields. Make sure this file is tab-delineated.'.format(str(linenum + 1)), file = sys.stderr)
            exit(1)
        ...
        try:
            ...
            print('ERROR: line #{} is out of order. This will give an incorrect deduplication. Use cmd "sort -k1,1 -k2,2n [fragment_file]" before bulk deduplication.'.format(str(linenum+1)),
                file = sys.stderr)
            exit(1)
